...
|--------|----------:|---------:|----------:|
| MiniCPM4-0.5B | 318-350 мс | **234-244 мс** | +30% |
| SmolLM2-360M | 347-373 мс | **285-304 мс** | +20% |
| Qwen3-0.6B | 488-578 мс | **391 мс** | +25% |
| DeepSeek-R1-1.5B | 509-661 мс | **380-432 мс** | +35% |
| Qwen3-1.7B | 541 мс | **447 мс** | +21% |
| SmolLM3-3B | 916-1043 мс | **708-735 мс** | +30% |
| Qwen3-4B | 1216 мс | **1110 мс** | +10% |

MiniCPM4-0.5B показывает рекордное ускорение **+86%** — эффективная архитектура максимально выигрывает от снижения PCIe задержки. DeepSeek-R1-1.5B W4A16 достигает **11 tok/s** с оптимизацией, делая reasoning-модели практичными на edge-устройствах. Qwen3-1.7B с оптимизацией достигает **~108% от native** (7.9 vs 7.42). **9 конфигураций LLM** из 7 семейств моделей.
...
| Модель | Задача | По умолч. (мс) | Оптимизировано (мс) | Ускорение |
|--------|--------|----------:|------------:|--------:|
| EdgeTAM prompt enc | Видеосегм. | 0.297 | **0.270** | +10% |
| EdgeTAM prompt mask | Видеосегм. | 0.765 | **0.732** | +4% |
| gtcrn | Шумоподавл. | 1.607 | **1.434** | +12% |
| 3D-Speaker ECAPA-TDNN | ID говорящ. | 4.006 | **3.889** | +3% |
| EdgeTAM mask decoder | Видеосегм. | 5.338 | **5.184** | +3% |
| 3D-Speaker Res2NetV2 | ID говорящ. | 5.534 | **5.459** | +1% |
| RAFT-stereo 256x640 | Стерео глуб. | 21.19 | **21.28** | ~0% |
| EdgeTAM image encoder | Видеосегм. | 23.88 | **23.73** | +1% |
| RAFT-stereo 384x1280 | Стерео глуб. | 112.55 | **112.40** | ~0% |
| IGEV++ (RTIGEV) | Стерео глуб. | 143.40 | **143.06** | ~0% |
...
**60+ моделей протестировано** в 21 категории: LLM, детекция, сегментация (инстанс/семантическая), классификация, OCR, распознавание/реставрация лиц, super-resolution, zero-shot, распознавание речи, TTS, стерео глубина, видеосегментация, ID говорящего, шумоподавление, трекинг, ключевые точки, QR-детекция, удаление фона, колоризация и другие.
