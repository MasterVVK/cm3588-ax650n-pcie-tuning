...
Слои VLM декодера: +32-62% — совпадает с паттерном LLM. Оценка decode: SmolVLM2 ~38→54 tok/s (+42%, native: 76.7), SmolVLM ~43→66 tok/s (+53%, native: 80), FastVLM ~22→29 tok/s (+28%, native: 34.8), InternVL2.5-1B ~21→32 tok/s (+52%, **совпадает с native**: 32).
